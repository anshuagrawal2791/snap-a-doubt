...
module.exports = {
    ...
    addFeedback: (req, res) => {
        Users.update({email: req.user.email}, {$push: {feedbacks: req.body.feedback}}, (err) => {
            if (err)
                return res.status(400).send(err);
            mailer.mail(config.app.emailId, {
                feedback: req.body.feedback,
                user: req.user
            }, 'New Feedback', (err, resp) => {
                console.log(resp)
                console.log(err)
    ...
    getWallPosts: (req, res) => {
        ...
        if (req.user.students && !req.user.students.includes(req.body.student_email))
            ...
            WallPosts.find({student_email: req.body.student_email}, (err, posts) => {
                if (err)
                    return res.status(400).send(err)
                return res.status(200).json({'posts': posts});
            })
    },
    getStudentProgresses: (req, res) => {
        if (!req.body.student_email)
            return res.status(400).send('student email is required');
        if (req.user.students && !req.user.students.includes(req.body.student_email))
            return res.status(403).send('not authorized to get this student\'s data. Assign student first');
            ...
            StudentProgresses.find({student_email: req.body.student_email}, (err, progresses) => {
                if (err)
                    return res.status(400).send(err)
